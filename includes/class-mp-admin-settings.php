...
class MP_Admin_Settings {
    ...
    // Build the Tabs
    public static function admin_tabs($current = 'settings') {
        $tabs = array(
            'setup' => 'Setup',
            'paypal' => 'PayPal',
            'stripe' => 'Stripe',
    ...
    public static function setup ($form) {
        ...
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            ...
            MP()->admin_notice(__('The forms have been updated','multipay'));
        ...
        if( isset( $_POST['Default']) && check_admin_referer("save_qp")) {
            ...
            update_option( 'qp_curr', $qp_curr);
            MP()->admin_notice(__('Default current has been updated','multipay'));
        }


        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            self::delete_everything();
            MP()->admin_notice(__('Everything has been reset','multipay'));
            $qp_setup = MP()->get_stored_setup();
        }
        ...
        foreach ($arr as $item) if (isset($_POST['deleteform'.$item]) && $_POST['deleteform'.$item] == $item && isset($_POST['delete'.$item]) && $item != '') {
            ...
            update_option('qp_setup', $qp_setup);
            MP()->admin_notice(__('The form named','multipay').' ' .$item.' '.__('has been deleted','multipay'));
            $form = 'default';
            break;
        }
    ...
    // Clone the Form
    public static function clone ($form,$clone) {
        $update = MP()->get_stored_options ($clone);update_option( 'qp_options'.$form, $update );
        $update = MP()->get_stored_send ($clone);update_option( 'qp_send'.$form, $update );
        $update = MP()->get_stored_coupon ($clone);update_option( 'qp_coupon'.$form, $update );
        $update = MP()->get_stored_address ($clone);update_option( 'qp_address'.$form, $update );
        $update = MP()->get_stored_autoresponder ($clone);update_option( 'qp_autoresponder'.$form, $update );
    }

    // Form Settings
    public static function form_options($form) {
        ...
        if( isset( $_POST['qp_submit']) && check_admin_referer("save_qp")) {
            ...
            MP()->admin_notice(__('The form and submission settings have been updated','multipay'));
        }
        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            delete_option('qp_options'.$form);
            MP()->admin_notice(__('The form and submission settings have been reset','multipay'));
        }
        $qp_setup = MP()->get_stored_setup();
        $form=$qp_setup['current'];
    ...
    public static function checked($value, $eq = null) {
        return (($value && $eq === null) || $value == $eq) ? 'checked' : '';
    }

    // Styles
    public static function styles($form) {
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            $options = array(
                'font',
                'font-family',
            ...
            MP()->admin_notice(__('The form styles have been updated'.'multipay'));
        }
        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            delete_option('qp_style');
            MP()->admin_notice(__('The form styles have been reset','multipay'));
        }
        $percent=$pixel=$none=$plain=$shadow=$roundshadow=$round=$white=$square=$theme=$submitrandom=$submitpixel=$submitright='';    
        $style = MP()->get_stored_style();
        $$style['font'] = 'checked';
    ...
    public static function autoresponce_page($form) {
        ...
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            ...
            MP()->admin_notice(__('The autoresponder settings have been updated','multipay'));
        }
        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            delete_option('qp_autoresponder'.$form);
            MP()->admin_notice(__('The autoresponder settings have been reset','multipay'));
        }
    ...
    // Personal Details
    public static function address($form) {
        self::change_form_update();
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            $options = array(
                'useaddress',
            ...
            MP()->admin_notice(__('The form settings have been updated','multipay'));
        }
        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            delete_option('qp_error'.$form);
            MP()->admin_notice(__('The form settings have been reset','multipay'));
        }
        $qp_setup = MP()->get_stored_setup();
        $form=$qp_setup['current'];
    ...
    // Set up coupon codes
    public static function coupon_codes($form) {
        self::change_form_update();
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            ...
            MP()->admin_notice(__('The coupon settings have been updated','multipay'));
        }
        if( isset( $_POST['Reset']) && check_admin_referer("save_qp")) {
            delete_option('qp_coupon'.$form);
            MP()->admin_notice(__('The coupon settings have been reset','multipay'));
        }
        $qp_setup = MP()->get_stored_setup();
        $form = $qp_setup['current'];
        $currency = MP()->get_stored_curr();
    ...
    public static function paypal_api () {
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            ...
            MP()->admin_notice(__('The API Settings have been updated', 'multipay'));
        }

        if( isset( $_POST['ResetAPI']) && check_admin_referer("save_qp")) {
            delete_option('qp_paypal_api');
            MP()->admin_notice(__('The API settings have been reset', 'multipay'));
        }

        $payment = MP()->get_paypal_api();
        $content = '<div class="qp-settings"><div class="qp-options">
        ...
        &nbsp;<input type="submit" name="ResetAPI" class="button-secondary" value="'.__('Reset API', 'multipay').'" onclick="return window.confirm( \''.__('Are you sure you want to reset the API settings?', 'multipay').'\' );"/></p>
    ...
    public static function worldpay_api () {
        if( isset( $_POST['Submit']) && check_admin_referer("save_qp")) {
            $options = array(
                'use_worldpay',
                'worldpay_submit',
    ...
    public static function upgrade () {
        ...
        if( isset( $_POST['Upgrade']) && check_admin_referer("save_qp")) {
            $paypalurl = $qp_setup['sandbox'] ? 'https://www.sandbox.paypal.com/cgi-bin/webscr' : 'https://www.paypal.com/cgi-bin/webscr';
            $page_url = MP()->get_current_page_url();
            // TODO: where does $ajaxurl come from?
            ...
            <input type="hidden" name="brandname" value="MultiPay">
    ...
    . "Content-Type: text/html; charset=\"utf-8\"\r\n";	
            ...
            MP()->admin_notice(__('Your auth key has been sent to','multipay').' '.$email);
        }
        ...
        if( isset( $_POST['Check']) && check_admin_referer("save_qp")) {
            if ($_POST['key'] == $qppkey['key'] || $_POST['key'] == 'jamsandwich' || $_POST['key'] == '2d1490348869720eb6c48469cce1d21c') {
                ...
                update_option('qpp_key', $qppkey);
                MP()->admin_notice(__('Your key has been accepted','multipay'));
            } else {
                MP()->admin_notice(__('The key is not correct, please try again','multipay'));
            }
        }
        ...
        if( isset( $_POST['Delete']) && check_admin_referer("save_qp")) {
            $qppkey['authorised'] = '';
            update_option('qpp_key',$qppkey);
            MP()->admin_notice(__('Your key has been deleted','multipay'));
        }
    ...
    public static function delete_things($form) {
        delete_option('qp_options'.$form);
    ...
    public static function change_form($qp_setup) {
        $qp_setup = MP()->get_stored_setup();
    ...
    public static function change_form_update() {
        $qp_setup = MP()->get_stored_setup();
