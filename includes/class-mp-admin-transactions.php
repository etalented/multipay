<?php
/**
 * MultiPay Admin Transactions
 *
 * @since   1.5
 * @class   MP_Admin_Transactions
 */
...
class MP_Admin_Transactions {
    ...
    public static function output() {
        ...
        <?php
        // The last, extra </div> is needed
    }

    public static function messages_admin_tabs($current = 'default') { 
        $qp_setup = MP()->get_stored_setup();
        $tabs = explode(",",$qp_setup['alternative']);
        sort($tabs);
        ...
        <nav class="nav-tab-wrapper">
            <?php foreach( $tabs as $tab ): ?>
                ...
                <?php $url = 'admin.php?page=multipay-transactions&tab=' . urlencode( $tab ); ?>
    ...
    public static function show_messages($form) {
        $qp_setup = MP()->get_stored_setup(); 
        $qp = MP()->get_stored_options($form);

        if( isset($_POST['qp_emaillist'])) {
            $message = get_option('qp_messages'.$form);
            $messageoptions = MP()->get_stored_msg();
            $content = MP()->messagetable ($form,'checked');
            ...
            MP()->admin_notice(__('Message list has been sent to','multipay').' '.$qp_email.'.');
        }
        ...
        if( isset( $_POST['Submit'])) {
            ...
            MP()->admin_notice(__('The messages options have been updated.','multipay'));
        }
        ...
        if( isset($_POST['qp_delete_selected'])) {
            ...
            MP()->admin_notice(__('Selected payments have been deleted.','multipay'));
        }
        ...
        $dashboard .= MP()->messagetable($form,'');
        if ( is_array(get_option('qp_messages'.$form) ) ) {
